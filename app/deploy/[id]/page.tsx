  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setMessages(prev =>
          prev.map(m =>
            m.id === aiMessage.id ? { ...m, typing: false } : m
          )
        )
      }, reply.length * 12 + 50)
  ...
  return (
    <div className={`h-screen flex flex-col ${selectedTheme.color}`}>
      ...
      <header className="bg-black/30 border-b border-white/20 px-4 py-3">
        <h1 className={`font-semibold ${selectedTheme.textColor}`}>
          {chatbot.name}
        </h1>
      </header>

      {/* CHAT */}
      <main className="flex-1 overflow-y-auto px-4 py-6 space-y-6 max-w-3xl mx-auto">
